    ...
    #define log_print(CON, MOD, COLOR, LVL, FMT, ...) \
        do {                                          \
            if (AOS_LOG_LEVEL & CON) {                \
                csp_printf(COLOR "[%s] <%s> %s  : ", get_current_time(), LVL, MOD); \
...
#define LOGE_IMPL(mod, fmt, ...) \
        log_print(AOS_LL_V_ERROR, mod, COL_RED, "E", fmt, ##__VA_ARGS__)
#define LOGW_IMPL(mod, fmt, ...) \
        log_print(AOS_LL_V_WARN,  mod, COL_YEL, "W", fmt, ##__VA_ARGS__)
#define LOGI_IMPL(mod, fmt, ...) \
        log_print(AOS_LL_V_INFO,  mod, COL_BLU, "I", fmt, ##__VA_ARGS__)
