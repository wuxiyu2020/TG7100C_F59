...
{
    BREEZE_INFO("breeze_post_ext_fast: ");
    hex_byte_dump_debug(buffer, length, 24);
    if (length == 0 || length > BZ_MAX_PAYLOAD_SIZE) {
        return BZ_EDATASIZE;
    }
...
void breeze_restart_advertising()
{
    ...
    ais_adv_init_t adv_data = {
        ...
        .name = { .ntype = AIS_ADV_NAME_FULL, .name = "E70BCZ01" },          //modify by zhoubw220616
    };
...
int breeze_start_advertising(uint8_t bind_state, uint8_t awss_flag, char* ad_name)
{
    uint32_t size;
    ais_adv_init_t adv_data = {
        .flag = AIS_AD_GENERAL | AIS_AD_NO_BREDR,
        .name = { .ntype = AIS_ADV_NAME_FULL, .name = ad_name }, //.name = { .ntype = AIS_ADV_NAME_FULL, .name = "TG" },
    };

    BREEZE_INFO("启动蓝牙广播...... (广播名称=%s awss_flag=%d bind_state=%d)", ad_name, awss_flag, bind_state);
