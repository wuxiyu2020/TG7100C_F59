...
#define CHECK_IMG_BUF_SIZE   512
        ...
        printf("[OTA] [TCP] aaa Update PARTITION, partition len is %lu\r\n", ptEntry.len);
...
int ota_upgrade_cb(void* pctx, char *json)
{
    ...
    if (0 == ota_parse(ctx, json)) {
        ...
        if(ctx->upg_module_flag == 1)
        {
            slist_for_each_entry(&(ctx->module_list), node, iotx_ota_module_info_t, next) 
            {
                ...
                // strcmp(node->device_name, device_name) == 0 &&
                    strcmp(node->module_name, ctx->module_info->module_name) == 0 && ctx->module_info != NULL) 
                {
                    ...
                    OTA_LOG_W("模块OTA: module=%s  new version=%s  old version=%s", ctx->module_info->module_name, ctx->module_info->module_version, node->module_version);
                    break;
                }
            }
        ...
        else
        {
            ...
            OTA_LOG_E("ota version %s, old version is %s!", ctx->ota_ver, ctx->sys_ver);
        }

        #endif
        // if(is_ota > 0) {
        if (is_ota != 0){           //modify by zhoubw
            ...
            OTA_LOG_E("ota version is same, discard it.");
