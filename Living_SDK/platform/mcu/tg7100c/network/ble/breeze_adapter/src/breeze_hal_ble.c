...
{
    if (err) {
        LOGW("aos_awss", "Connection failed! (err %u)", err);
    } else {
        LOGW("aos_awss", "Connected!");
        g_conn = conn;
...
{
    LOGW("aos_awss", "Disconnected! (reason %u)!", reason);
    g_conn = NULL;
    pre_vdata_len = 0;
                                   ...
                                   uint16_t                   value)
{
    ...
    switch (value) {
        case BT_GATT_CCC_NOTIFY:
            LOGW("aos_awss", "CCC cfg changed to NOTIFY (%d)!", value);
            val = AIS_CCC_VALUE_NOTIFY;
            break;
        default:
            LOGW("aos_awss", "%s CCC cfg changed to %d!", __func__, value);
                                   ...
                                   uint16_t                   value)
{
    ...
    switch (value) {
        case BT_GATT_CCC_INDICATE:
            LOGW("aos_awss", "CCC cfg changed to INDICATE (%d)!", value);
...
ais_err_t ble_advertising_start(ais_adv_init_t *adv)
{
    ...
    //LOGW("ble adv: name[%d] = %s\r\n", sd[0].data_len, sd[0].data);
    err = bt_le_adv_start(&param, ad, ad_len, sd, sd_len);
    if (err) {
        BT_ERR("Advertising failed to start (error %d)\n", err);
...
ais_err_t ble_get_mac(uint8_t *mac)
{
    ...
    if (err != AIS_ERR_SUCCESS) {
        BT_INFO("Failed to get local addr.\r\n");
    } else {
        memcpy(mac, laddr.a.val, 6);
        BT_INFO("Local addr got (%02x:%02x:%02x:%02x:%02x:%02x).\n", mac[0],
